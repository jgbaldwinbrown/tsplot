...
main = function() {
	...
	data = data[data$Name != "unbitted_unsel",]
	data$Treatment = factor(data$Treatment, levels=c("White pigeons", "Black pigeons", "Runt", "Figurita"))
	data$Name = factor(data$Name, levels=c("unbitted", "bitted"))
	# data$Name = factor(data$Name, levels=c("unbitted", "bitted", "unbitted_unsel"))

	breaks = c("unbitted", "bitted")
